...
public class UIButton extends UIComponent {
    ...
    public void draw(final int mouseX, final int mouseY) {
        if (this.visible) {
            ...
            GuiUtils.drawTexturedModalRect(0, 0, 0, 46 + offsetV * 20, parent.getWidth() / 2,
                    parent.getHeight(), 0);
            GuiUtils.drawTexturedModalRect(parent.getWidth() / 2, 0, 200 - parent.getWidth() / 2,
                    46 + offsetV * 20, parent.getWidth() / 2, parent.getHeight(), 0);
            final int colorUsed = enabled
                    ? (parent.isHovered() ? DEFAULT_HOVER_COLOR : DEFAULT_COLOR)
                    : DEFAULT_DISABLED_COLOR;
            DrawUtil.drawCenteredString(fontrenderer, this.text, parent.getWidth() / 2,
                    (parent.getHeight() - 8) / 2, colorUsed);
        }
    }
